...
for asset in data.get("assets", []):
    if pattern.fullmatch(asset["name"]):
        print(asset["browser_download_url"]); print(data["tag_name"], file=sys.stderr); break
else: raise SystemExit("未找到匹配资源："+sys.argv[1])' "$pattern"
}

MIHOMO_META="$WORK_DIR/mihomo-meta"
MIHOMO_URL=${MIHOMO_URL:-$(asset_url MetaCubeX/mihomo 'mihomo-android-arm64-v8-v[^/]+\.gz' 2>"$MIHOMO_META")}
...
cp "$WORK_DIR/mihomo.gz" "$OUTPUT_DIR/mihomo-android-arm64-v8.gz"
CORE_SHA=$(sha256sum "$WORK_DIR/mihomo.gz" | cut -d' ' -f1)
CORE_SIZE=$(wc -c < "$WORK_DIR/mihomo.gz" | tr -d ' ')
CORE_VERSION=$(sed -n '1p' "$MIHOMO_META")
cat > "$OUTPUT_DIR/mihomo-manifest-arm64-v8a.json" <<EOF
{"version":"${CORE_VERSION#v}","abi":"arm64-v8a","url":"https://github.com/liuyi-htu/Mclash/releases/latest/download/mihomo-android-arm64-v8.gz","sha256":"$CORE_SHA","size":$CORE_SIZE,"publishedAt":"$(date -u +%FT%TZ)"}
EOF
sha256sum "$OUTPUT_DIR/Mclash-root-arm64-v8a.apk" > "$OUTPUT_DIR/Mclash-root-arm64-v8a.apk.sha256"
