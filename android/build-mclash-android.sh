...
ABI=arm64-v8a
DEFAULT_CONFIG_FILE="$SOURCE_DIR/../assets/default-config.yaml"
...
[ -s "$DEFAULT_CONFIG_FILE" ] || {
  echo "缺少内置默认 Mihomo 配置：$DEFAULT_CONFIG_FILE" >&2
  exit 1
}
[ "$(wc -c < "$DEFAULT_CONFIG_FILE")" -le 8388608 ] || {
  echo "内置默认 Mihomo 配置超过 8 MB" >&2
  exit 1
}
grep -Eq '^[[:space:]]*(proxies|proxy-providers|proxy-groups|rules|mixed-port|port|socks-port|redir-port|tproxy-port)[[:space:]]*:' \
  "$DEFAULT_CONFIG_FILE" || {
  echo "内置文件不是 Mihomo/Clash YAML 配置" >&2
  exit 1
}
